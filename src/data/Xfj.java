...
public class Xfj {
    ...
    private static List<Map.Entry<String, Double>> get_in(List<Plan> plan_nodes, List<Score> score_nodes, List<String> term_id_list, boolean use_plan_term){
        ...
        Map<String, List<Plan>> year_plans_map = new HashMap<>();
        ...
        for (Plan plan : plan_nodes){
            ...
            try {
                year_code_of_plan = plan.getFinal_term().substring(0, UrlProcess.isInternational()?4:9);
            }catch (Exception e){
                LogMe.e("Xfj", "Exception caught. Roll back to 4");
                ...
                for (Plan p : plans_list_of_year){
                    if (p.getSterm() != null && !p.getSterm().isEmpty()){//如果有入选成绩，照抄入选成绩
                        ...
                        List<Score> my_scores = score_nodes.stream()
                                .filter(score -> score.getCourse_id().equals(p.getPlan_course_id())).sorted(new Comparator<Score>() {
                                    @Override
    ...
    public static List<Map.Entry<String, Double>> get_must_on_background(boolean use_plan_term){
        ...
        gradesList = gradesList.stream().filter(new Predicate<Grades>() {
            ...
            public boolean test(Grades grades) {
                return grades.courseid.toLowerCase().startsWith("b".toLowerCase()) || grades.courseid.toLowerCase().startsWith("x".toLowerCase());
            }
        }).collect(Collectors.toList());
        ...
        for (Grades grade : gradesList){
            ...
            if (grade.courseid.toLowerCase().startsWith("xz".toLowerCase())){
                if (xz_map.containsKey(grade.courseid)){
                    xz_map.get(grade.courseid).add(grade);
                }else {
                    xz_map.put(grade.courseid, new LinkedList<Grades>(){{add(grade);}});
                }
            }
        }
